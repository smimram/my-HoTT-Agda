...
module core.Path where
  ...
  ap2 : ∀ {ℓ₀ ℓ₁ ℓ₂} {A : Type ℓ₀} {B : Type ℓ₁} {C : Type ℓ₂}
    → (f : A → B → C) {a a' : A} {b b' : B}
    → a ≡ a' → b ≡ b'
    → f a b ≡ f a' b'
  ...
  happly : ∀ {ℓ₀ ℓ₁} {A : Type ℓ₀} {B : A → Type ℓ₁}
    → {f g : (x : A) → B x} (p : f ≡ g)
    → (x : A) → f x ≡ g x
  happly refl x = refl
  ...
  transport : ∀ {ℓ₀ ℓ₁} {A : Type ℓ₀} (B : A → Type ℓ₁)
    → {x y : A} → x ≡ y
    → B x → B y
